...
const FetchRides = () => {
    ...
    useEffect(() => {
        const getRides = async () => {
            ...
            setErrMsg(null);
            try{
                const res = await axios.get(`${import.meta.env.VITE_LOCAL_API_URL}/api/fetch_rides_by_date/${searchDate}`);
    ...
    return (
        <section className='message_section'>
            <h2>Shuttle Rides on "{searchDate}"</h2>
            <div className='msg_buttons'>
                <ButtonGroup className='button_group' aria-label="Basic example">
                    <Button className="msg_button" onClick={handleToday}>Today's Rides ({moment().format("YYYY-MM-DD")})</Button>
                    <Button className="msg_button" onClick={handleTomorrow}>Tommorow's Rides ({moment().add(1, 'days').format("YYYY-MM-DD")})</Button>
                    <Button className="msg_button" onClick={() => setShowDateInput(!showDateInput)}>Custom Date Rides</Button>
                </ButtonGroup>
            </div>
            ...
            {Object.entries(rides).map(([route, timesData]) => {
                ...
                <div key={route} className="route-section">
                    ...
                    /* 2) If timesData is an object (like { '5:00 AM': [ ... ], '12:00 PM': [ ... ] }) */
                            ...
                            <table className="rides_table">
                                ...
                                <tbody>
                                    ...
                                    trips.map((trip, i) => (
                                        <tr key={i}>
                                        <td>{trip.trip_type || "N/A"}</td>
                                        <td>{trip.ticket_id ? (<Link className="box_link" to={"/ride_details/"+trip.ticket_id}>{trip.ticket_id || "N/A"}</Link>) : "N/A"}</td>
                                        <td>{trip.pick_up || "N/A"}</td>
                                        <td>{trip.drop_off || "N/A"}</td>
                                        {/* <td>{trip.airline || "N/A"}</td> */}
                                        <td>{trip.traveler_count || "N/A"}</td>
